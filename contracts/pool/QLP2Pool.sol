...
contract QLP2Pool is LPTokenWrapper2Q, AdminRole {
    ...
    function getTvl() public view returns (uint256) {
        return LiquidityOracle(liquidityOracle).tvl();
    }
    ...
    function getReward() public updateReward(msg.sender) checkhalve checkStart {
        ...
        if (reward > 0) {
            ...
            uint256 tvl = getTvl();
            if (tvl < minTVL) {
                uint256 userReward = reward.mul(tvl).div(minTVL);
                uint256 fundReward = reward.sub(userReward);
                sShare.safeTransfer(msg.sender, userReward);
                sShare.safeTransfer(fund, fundReward);
            } else {
